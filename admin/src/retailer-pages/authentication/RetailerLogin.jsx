...
import { useLoginMutation, useRetailerLoginMutation } from "../../features/api/apiSlice";
...
function RetailerLogin() {
  ...
  const [retailerLogin, { isLoading, error }] = useRetailerLoginMutation();
  ...
  const handleLogin = async () => {
    ...
    try {
      const res = await retailerLogin({ email, password }).unwrap(); // unwrap to handle errors
      localStorage.setItem("retailerName", res.retailerUser.fullName);
      // navigate("/gemstone/admin/");
      window.location = `${baseUrl}retailer/dashboard`;
    } catch (error) {
      setErrorMessage(error?.data?.msg || error?.data?.message || error?.error || "Login failed. Try again later.");
    }
  };
          ...
          >
            <div className="flex justify-between items-center">
              ...
              <motion.p
                ...
                onClick={() => navigate("/retailer/forgot-password")}
              >
                Forgot Password?
              </motion.p>
            </div>
            ...
            {error && (
              ...
              >
                {error.data?.message || error?.data?.msg || error?.error || "An error occurred. Please try again."}
              </motion.p>
            )}
          </motion.div>
